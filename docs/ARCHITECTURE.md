...
8. **Admin route protection via external auth microservice** — admin operations (the whole `/api/v1/admin/**` tree, market rates included) delegate token validation to an external user service via `ExternalAuthPort` / `ExternalAuthAdapter`: the adapter calls `GET <AUTH_EXTERNAL_URL>/introspect` forwarding the `Authorization: Bearer <token>` header as-is; a 200 response means the token is valid; 401/403 and any I/O error fail closed (the adapter returns `false`). The boundary between BillMind and the auth service is the port — swapping the external service requires only a new adapter, not changes to the filter or domain.
...
14. **Untrusted prompt content is fenced with a per-request nonce, never with a fixed delimiter.** Every untrusted string reaching an LLM goes through `_shared/infrastructure/llm/prompt/` — `PromptFence` for blocks, `PromptText` for short inline values. A fixed delimiter (the old `<<< >>>`, the old `--- FIN CONTEXTO REGULATORIO ---`) is guessable by content authored *before* the request: an ingested BOE PDF or a Kafka row can embed the closing marker and everything after it reads as system-level instruction. `PromptFence.random()` draws an 8-hex-char nonce per prompt build and stamps it into both markers, so predating content cannot reproduce it — that, not the sanitizing, is the boundary. **The two classes solve different problems:** a fence stops content *escaping* its block; `PromptText` stops content *forging structure inside* the block (the market list is line-based, so a newline in a company name fabricates an extra tariff row). Note the guarantee is structural, not semantic — flattening demotes forged rows to inert text without deleting it, and no delimiter makes a model ignore an instruction that sits legitimately inside its block. That residual is countered by three other choices: retrieved data lives in the `user` role, never `system` (`LlmAssistantAdapter`; the agentic path gets this free via tool results), the data block is followed by a trailing instruction block (the sandwich), and invoice extraction is typed — `ElectricityFields` and friends are all `BigDecimal`/`LocalDate`, so the anonymous upload path carries no free text into a prompt at all.
15. **Provider failures are classified once, at the only point every LLM call crosses.** `TimedChatLanguageModel.chat()` already wrapped every model call for telemetry, so that is where `LlmFailures.translate()` turns whatever a provider threw into one of two domain failures: `LlmRateLimitedException` (**429**, carrying the wait the provider named) or `LlmServiceUnavailableException` (**503**). The alternative — each adapter catching `RuntimeException` and wrapping it — is what the code did before, and it was wrong twice over: it was fan-out (four call sites in `invoice/` had to know a provider's exception vocabulary), and it *flattened* the distinction, so a throttle reached the user as "the service is down" or, off the invoice path, as a bare **500**. Those four `catch` blocks are now gone. **The classification is deliberately narrow:** only throttles and outages are translated; everything else propagates untouched, which is what keeps `InvalidRequestException` reaching the agentic loop, where a malformed tool call is a bug to recover from rather than an incident to report. Detection reads the whole cause chain (providers nest the real failure two or three deep) and trusts wording — `rate limit`, `quota`, `resource_exhausted` — **only on an `HttpException`**, since a model answering a question about tariffs will happily write "rate limit" into its prose. Two consequences follow. First, **the provider's raw failure never reaches the client**: it is kept as the `cause` for the logs (where `%pii` scrubs it, because an over-length 400 can echo invoice text back), while every handler builds the body from `getMessage()`, which is the fixed Spanish string. Second, **SSE needs its own channel**: `POST /assistant/chat` commits its HTTP status the moment the stream opens, so failures leave as an `error` event carrying a `code` (`RATE_LIMITED` / `LLM_UNAVAILABLE` / `INTERNAL`) and the stream is then closed with `complete()` — closing with `completeWithError()` surfaces in the browser as a network failure and hides the message just sent. The agentic adapter re-throws these two rather than degrading them into its generic fallback answer: "wait a bit" and "I couldn't answer that" are different things, and only the first tells the user what to do next. See `docs/RATELIMIT.md` → *The other throttle* for the wait vocabulary and the UI's side of the contract.
...
|---|---|---|---|
| `/api/v1/admin/**` (market rates included, reads included) | `ADMIN` | required | — |
| `POST /api/v1/invoices`, `POST /api/v1/assistant/chat`, … | `ANONYMOUS` | — | required |
| `/actuator/**`, `/chat/**`, `/market-rates.html` | `OPEN` | — | — |

`ADMIN` is decided by one rule — the `/api/v1/admin/**` prefix — and nothing else. An endpoint that needs guarding is *moved into that tree* rather than registered in a list beside it, which is why market rates were relocated from `/api/v1/market-rates` to `/api/v1/admin/market-rates` when their reads stopped being public: a guard expressed in the URL cannot fall out of step with it, and no verb added there can be born open. Nothing under `/api/v1/**` is `OPEN` any more.
